...
function errorResponse(message: string, status = 500) {
  console.error(`[plant-diagnosis] ERROR (${status}):`, message);
...
// ─── PlantNet ─────────────────────────────────────────────────────────────────
async function identifyWithPlantNet(imageBase64: string): Promise<Record<string, unknown>> {
  console.log("[PlantNet] Starting plant identification...");

  if (!PLANTNET_API_KEY) {
    throw new Error("PLANTNET_API_KEY secret is not set in Supabase dashboard.");
  }
  ...
  try {
    ...
    throw new Error(`Base64 decode failed: ${decodeErr instanceof Error ? decodeErr.message : decodeErr}`);
  }
  ...
  try {
    ...
    throw new Error(`PlantNet network error: ${fetchErr instanceof Error ? fetchErr.message : fetchErr}`);
  }
  ...
  console.log("[PlantNet] Success. Top result:", JSON.stringify(json?.results?.[0]?.species?.scientificName ?? "N/A"));
  return json;
}

// ─── Plant.id Health Assessment ───────────────────────────────────────────────
async function assessWithPlantId(imageBase64: string): Promise<Record<string, unknown>> {
  console.log("[Plant.id] Starting health assessment...");

  if (!PLANTID_API_KEY) {
    throw new Error("PLANTID_API_KEY secret is not set in Supabase dashboard.");
  }
  ...
  let res: Response;
  try {
    res = await fetch(PLANTID_ENDPOINT, {
      method: "POST",
      headers: {
    ...
    throw new Error(`Plant.id network error: ${fetchErr instanceof Error ? fetchErr.message : fetchErr}`);
  }
  ...
  const json = await res.json();
  const isHealthy = (json?.result as Record<string, unknown>)?.is_healthy;
  console.log("[Plant.id] Success. is_healthy:", JSON.stringify(isHealthy));
  return json;
}

// ─── Plant.id Identification Fallback (when PlantNet fails) ──────────────────
async function identifyWithPlantId(imageBase64: string): Promise<Record<string, unknown>> {
  ...
  let res: Response;
  try {
    res = await fetch(PLANTID_IDENTIFY_ENDPOINT, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Api-Key": PLANTID_API_KEY,
    ...
    throw new Error(`Plant.id identification network error: ${fetchErr instanceof Error ? fetchErr.message : fetchErr}`);
  }
  ...
  const json = await res.json();
  console.log("[Plant.id-ID] Success. Top suggestion:",
    JSON.stringify((json?.result?.classification?.suggestions as Array<Record<string, unknown>>)?.[0]?.name ?? "N/A"));
  return json;
}

// ─── Parse Plant.id Identification result into PlantNet-compatible shape ──────
function parsePlantIdIdentification(data: Record<string, unknown>) {
...
// ─── Parse PlantNet ────────────────────────────────────────────────────────────
function parsePlantNet(data: Record<string, unknown>) {
  ...
  const species = best.species as Record<string, unknown>;
  return {
    scientificName: (species?.scientificName as string) ?? "Unknown",
    commonNames: (species?.commonNames as string[]) ?? ["Unknown plant"],
    family: ((species?.family as Record<string, unknown>)?.scientificName as string) ?? "Unknown",
    genus: ((species?.genus as Record<string, unknown>)?.scientificName as string) ?? "Unknown",
    confidence: Math.round(((best.score as number) ?? 0) * 100),
  };
}

// ─── Parse Plant.id ────────────────────────────────────────────────────────────
function parsePlantId(data: Record<string, unknown>) {
  const result = data.result as Record<string, unknown>;
  const isHealthy = ((result?.is_healthy as Record<string, unknown>)?.probability as number ?? 0) > 0.5;
  const healthProbability = Math.round(((result?.is_healthy as Record<string, unknown>)?.probability as number ?? 0) * 100);
  const disease = result?.disease as Record<string, unknown>;
  const suggestions = (disease?.suggestions as Array<Record<string, unknown>>) ?? [];
  const top = suggestions[0];
  return {
    isHealthy,
    healthProbability,
    topDiseaseName: (top?.name as string) ?? "None detected",
    topDiseaseConfidence: Math.round(((top?.probability as number) ?? 0) * 100),
    topDiseaseDescription: ((top?.details as Record<string, unknown>)?.description as string) ?? "",
    biological: (((top?.details as Record<string, unknown>)?.treatment as Record<string, unknown>)?.biological as string[]) ?? [],
    chemical: (((top?.details as Record<string, unknown>)?.treatment as Record<string, unknown>)?.chemical as string[]) ?? [],
    prevention: (((top?.details as Record<string, unknown>)?.treatment as Record<string, unknown>)?.prevention as string[]) ?? [],
    classification: ((top?.details as Record<string, unknown>)?.classification as string[]) ?? [],
    commonDiseaseNames: ((top?.details as Record<string, unknown>)?.common_names as string[]) ?? [],
  };
}
...
async function sanitizeWithGroq(
  ...
  if (!GROQ_API_KEY) {
    throw new Error("GROQ_API_KEY secret is not set in Supabase dashboard.");
  }
...
Return ONLY the JSON object. No extra text, no markdown fences.`;
  ...
  try {
    ...
    throw new Error(`Groq network error: ${fetchErr instanceof Error ? fetchErr.message : fetchErr}`);
  }
...
serve(async (req: Request) => {
  ...
  console.log("[plant-diagnosis] ===== Request received =====");
  console.log("[plant-diagnosis] Time:", new Date().toISOString());
  console.log("[plant-diagnosis] Secrets — PLANTNET:", PLANTNET_API_KEY ? "SET" : "MISSING",
    "| PLANTID:", PLANTID_API_KEY ? "SET" : "MISSING",
  ...
  try {
    ...
    console.log("[plant-diagnosis] imageBase64 length:", imageBase64?.length ?? 0);
  } catch (bodyErr) {
    return errorResponse(`Failed to parse request body: ${bodyErr instanceof Error ? bodyErr.message : bodyErr}`, 400);
  }

  if (!imageBase64 || imageBase64.length < 100) {
    return errorResponse("imageBase64 is required and must be a valid base64 image string.", 400);
  }

  // ── Step 1 + 2: PlantNet & Plant.id in PARALLEL ───────────────────────────
  // Both APIs receive the same base64 simultaneously — saves 3-5 seconds vs sequential.
  ...
  if (plantNetResult.status === "fulfilled") {
    ...
    console.warn("[plant-diagnosis] PlantNet failed:", plantNetResult.reason?.message ?? plantNetResult.reason);
  ...
  if (plantIdResult.status === "fulfilled") {
    ...
    console.error("[plant-diagnosis] Plant.id failed:", plantIdResult.reason?.message ?? plantIdResult.reason);
    return errorResponse(`Plant.id step failed: ${plantIdResult.reason?.message ?? plantIdResult.reason}`);
  }
  ...
  try {
    ...
    return errorResponse(`Groq step failed: ${err instanceof Error ? err.message : err}`);
  }
