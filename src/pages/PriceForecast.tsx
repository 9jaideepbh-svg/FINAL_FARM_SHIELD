...
export default function PriceForecast() {
  ...
  useEffect(() => {
    ...
    const matches = indianCities.filter(c => c.toLowerCase().includes(q)).slice(0, 5);
    ...
    return (
      <Layout>
        <div className="container max-w-lg mx-auto py-8 px-4">
          ...
          <p className="text-muted-foreground text-center mb-8">Know fair market prices before you sell</p>
                ...
                <div className="absolute top-full left-0 right-0 z-10 mt-1 border rounded-lg bg-card shadow-lg">
                  {locationSuggestions.map((s) => (
                    <button
                      ...
                      onClick={() => { setLocation(s); setLocationSuggestions([]); }}
                    >
      ...
      <Layout>
        <div className="container max-w-2xl mx-auto py-6 px-4">
          ...
          <div className="flex items-center gap-3 mb-6">
            <Button variant="ghost" size="icon" onClick={() => setScreen("location")}>
          ...
          <div className="relative mb-4">
            <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
            <Input
              placeholder="Enter crop name (English or Hindi)"
              value={cropSearch}
              onChange={(e) => setCropSearch(e.target.value)}
              className="pl-10 pr-10"
            />
            <button
              className={`absolute right-3 top-2.5 p-0.5 rounded-full ${isListening ? "text-red-500 animate-pulse" : "text-muted-foreground"}`}
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mb-4 scrollbar-none">
            {cropCategories.map(cat => (
              <Button
                key={cat.id}
                variant={cropCategory === cat.id ? "default" : "outline"}
                size="sm"
  ...
  return (
    <Layout>
      <div className="container max-w-2xl mx-auto py-6 px-4">
        {/* Header */}
        <div className="flex items-center gap-3 mb-6">
          <Button variant="ghost" size="icon" onClick={() => { setScreen("crop"); setResult(null); }}>
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div>
            <h1 className="text-xl font-bold">{selectedCrop?.emoji} {selectedCrop?.name} — {location}</h1>
            {result && <p className="text-xs text-muted-foreground">Updated: {result.data_freshness}</p>}
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-muted-foreground">Fetching latest prices...</p>
            <p className="text-xs text-muted-foreground">Analyzing market data, weather & demand</p>
          </div>
        ) : result ? (
          <div className="space-y-4">
            {/* Section B: Current Price */}
            <Card>
            ...
            {/* Section C: Nearby Markets */}
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base">📍 Prices in Nearby Markets</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            {/* Section D: Demand Forecast */}
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center gap-3 mb-3">
                  {getDemandIcon(result.demand_forecast.level)}
                  <Badge className={getDemandColor(result.demand_forecast.level)}>
                    {result.demand_forecast.level} DEMAND
                  </Badge>
                ...
                <ul className="space-y-1">
                  {result.demand_forecast.reasons.map((r, i) => (
            ...
            {/* Section E: Weather Impact */}
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  {result.weather_impact.rainfall_mm > 20 ? <CloudRain className="h-5 w-5 text-blue-500" /> : <Sun className="h-5 w-5 text-yellow-500" />}
                  Weather Impact
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-1 text-sm">
                  ...
                  {result.weather_impact.rainfall_mm > 0 && (
                    <p><span className="font-medium">Rainfall:</span> {result.weather_impact.rainfall_mm}mm expected</p>
                  )}
            ...
            <Card className="border-2 border-primary/30">
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  <DollarSign className="h-5 w-5 text-primary" /> Negotiation Guide
                </CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className={result.recommendation.action === "WAIT" ? "bg-yellow-50 border-yellow-200" : "bg-green-50 border-green-200"}>
              <CardContent className="pt-6">
                ...
                {result.recommendation.best_selling_window && (
                  <p className="text-sm font-medium">Best selling window: {result.recommendation.best_selling_window}</p>
                )}
              </CardContent>
            </Card>

            {/* Section I: Negotiation Tips (Expandable) */}
            <Card>
              <button
            ...
            <div className="flex gap-3">
              <Button variant="outline" className="flex-1" onClick={() => { setScreen("crop"); setResult(null); setSelectedCrop(null); }}>
                Search Another Crop
              </Button>
            </div>
          </div>
