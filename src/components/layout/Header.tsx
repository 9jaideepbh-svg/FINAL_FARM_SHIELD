    ...
    <header className="fixed top-0 left-0 right-0 z-50 w-full pt-6 px-4">
      <div className="container max-w-[1600px] mx-auto p-0">
        <GlassSurface width="100%" height="auto" borderRadius={40} opacity={0.7} backgroundOpacity={0.2} distortionScale={0} blur={20} className="w-full shadow-lg border border-white/20">
          <div className="flex h-16 md:h-20 items-center justify-between px-6 md:px-8 w-full gap-4 xl:gap-8">
            {/* Logo */}
            <Link to="/" className="flex items-center gap-2 shrink-0 group xl:mr-4">
            ...
            {/* Desktop Navigation */}
            <nav className="hidden xl:flex items-center xl:justify-start 2xl:justify-center gap-3 xl:gap-4 2xl:gap-5 flex-1 px-4 overflow-hidden">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  to={link.href}
